...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (verified === 'true') {
      toast.success(t('login.verifiedSuccess') || 'Email verified successfully! You can now sign in.');
    ...
    if (error === 'verification_failed') {
      toast.error(t('login.verificationFailed') || 'Email verification failed. Please try again.');
      router.replace(`/${locale}/login`);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = error?.message || t('login.error') || 'Invalid email or password';
  ...
  const handleResendEmail = async () => {
    ...
    try {
      ...
      toast.error(error.message || t('login.resendError') || 'Failed to send email');
    } finally {
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4">
          {authError?.code === 'email_not_confirmed' && (
            <Alert variant="destructive">
              ...
              <AlertTitle>{t('login.emailNotConfirmedTitle') || 'Email Not Verified'}</AlertTitle>
              <AlertDescription className="space-y-2">
                <p>{t('login.emailNotConfirmedDescription') || 'Your email address has not been verified yet. Please check your inbox and click the verification link.'}</p>
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
              ...
              <AlertDescription className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <Button
                    ...
                    className="w-full"
                  >
                    {resendingOtpEmail && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
        ...
        <CardFooter className="flex flex-col space-y-4">
          ...
          <p className="text-sm text-muted-foreground text-center">
            {t('login.noAccount')}{' '}
            <a href={`/${locale}/signup`} className="text-primary hover:underline">
              {t('login.signUp')}
            </a>
          </p>
        </CardFooter>
