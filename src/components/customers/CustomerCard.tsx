...
export function CustomerCard({ customer, locale = 'en' }: CustomerCardProps) {
  const hasDebt = customer.balance > 0;

  return (
    <Link href={`/${locale}/app/customers/${customer.id}`}>
      <Card className="border-[var(--color-border)] hover:shadow-md hover:border-[var(--color-accent)] transition-all duration-200 cursor-pointer">
