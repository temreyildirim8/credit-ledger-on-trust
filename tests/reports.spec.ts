...
/**
 * E2E Tests for Global Ledger Reports Page
...
test.describe('Reports Page', () => {
  ...
  test.describe('Reports Empty State', () => {
    ...
    test('should display empty state when no transactions exist', async ({ page }) => {
      ...
      const emptyStateVisible = await page.getByText(/no.*transaction|no.*data|start.*tracking|empty/i).isVisible().catch(() => false);
      const reportsVisible = await page.getByText(/shop.*summary|total.*owed|export/i).isVisible().catch(() => false);
    ...
    test('should show call to action in empty state', async ({ page }) => {
      ...
      const emptyStateVisible = await page.getByText(/no.*transaction|no.*data|start.*tracking/i).isVisible().catch(() => false);

      if (emptyStateVisible) {
        // Should have a button to go to dashboard
        const actionButton = page.getByRole('link', { name: /dashboard|get.*started|start/i });
  ...
  test.describe('Time Filter Tabs', () => {
    ...
    test('should display time filter tabs when data exists', async ({ page }) => {
      if (await skipIfUnauthenticated(page)) return;

      const hasData = await page.getByText(/shop.*summary|total.*owed|export/i).isVisible().catch(() => false);
  ...
  test.describe('Shop Summary Report', () => {
    ...
    test('should display shop summary cards when data exists', async ({ page }) => {
      if (await skipIfUnauthenticated(page)) return;

      const hasData = await page.getByText(/shop.*summary|total.*owed|collected/i).isVisible().catch(() => false);
    ...
    test('should display Total Owed stat', async ({ page }) => {
      ...
      const hasData = await page.getByText(/total.*owed|owed/i).isVisible().catch(() => false);
    ...
    test('should display Collected stat', async ({ page }) => {
      ...
      const hasData = await page.getByText(/collected|payment/i).isVisible().catch(() => false);

      if (hasData) {
        const collected = page.getByText(/collected/i);
        const hasCollected = await collected.first().isVisible().catch(() => false);
    ...
    test('should display Active Customers stat', async ({ page }) => {
      ...
      const hasData = await page.getByText(/active.*customer|customer/i).isVisible().catch(() => false);

      if (hasData) {
        const activeCustomers = page.getByText(/active.*customer/i);
        const hasCustomers = await activeCustomers.isVisible().catch(() => false);
    ...
    test('should display Collection Rate stat', async ({ page }) => {
      ...
      const hasData = await page.getByText(/collection.*rate|rate/i).isVisible().catch(() => false);
  ...
  test.describe('Debt Aging Report', () => {
    ...
    test('should display debt aging section when data exists', async ({ page }) => {
      if (await skipIfUnauthenticated(page)) return;

      const hasData = await page.getByText(/debt.*aging|aging|outstanding/i).isVisible().catch(() => false);
    ...
    test('should display aging categories', async ({ page }) => {
      ...
      const hasData = await page.getByText(/debt.*aging|aging|current|30.*day|60.*day|90.*day/i).isVisible().catch(() => false);
  ...
  test.describe('CSV Export', () => {
    ...
    test('should display export section when data exists', async ({ page }) => {
      ...
      const hasData = await page.getByText(/export|download/i).isVisible().catch(() => false);
    ...
    test('should trigger CSV download when button is clicked', async ({ page }) => {
      if (await skipIfUnauthenticated(page)) return;

      const hasData = await page.getByRole('button', { name: /csv|export.*csv/i }).isVisible().catch(() => false);
  ...
  test.describe('PDF Statement Generation', () => {
    ...
    test('should display PDF export button (may be disabled)', async ({ page }) => {
      if (await skipIfUnauthenticated(page)) return;

      const hasData = await page.getByText(/export|download|pdf/i).isVisible().catch(() => false);
    ...
    test('should display customer statement option', async ({ page }) => {
      ...
      const hasData = await page.getByText(/export|statement|customer/i).isVisible().catch(() => false);

      if (hasData) {
        const statementButton = page.getByRole('button', { name: /customer.*statement|statement/i });
        const hasStatement = await statementButton.isVisible().catch(() => false);
  ...
  test.describe('Responsive Design', () => {
    ...
    test('should display empty state on mobile', async ({ page }) => {
      ...
      // Either empty state or reports should be visible
      const contentVisible = await page.getByRole('heading', { name: /reports|analytics/i }).isVisible();
    ...
    test('should display export buttons on mobile', async ({ page }) => {
      ...
      if (hasData) {
        ...
        const exportButtons = page.getByRole('button', { name: /export|download|csv|pdf/i });
