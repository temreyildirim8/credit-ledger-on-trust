...
/**
 * Accessibility Tests for Ledgerly
 *
...
test.describe("Accessibility Tests - Images", () => {
  test("all images on home page should have alt text", async ({ page }) => {
    ...
    if (imagesWithoutAlt.length > 0) {
      console.log(
        `Images without alt text on home page: ${imagesWithoutAlt.length}`,
      );
    }
  ...
  test("all images on pricing page should have alt text", async ({ page }) => {
    ...
    if (imagesWithoutAlt.length > 0) {
      console.log(
        `Images without alt text on pricing page: ${imagesWithoutAlt.length}`,
      );
    }
...
test.describe("Accessibility Tests - Forms", () => {
  ...
  test("contact form inputs should have labels", async ({ page }) => {
    ...
    if (hasForm) {
      ...
      for (const input of inputs) {
        ...
        expect(
          hasLabel || (await input.getAttribute("type")) === "submit",
        ).toBe(true);
      }
    }
  });
  ...
  test("form inputs should have autocomplete attributes where appropriate", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/login`);
    await page.waitForLoadState("networkidle");
...
test.describe("Accessibility Tests - Buttons", () => {
  test("all buttons on home page should have accessible names", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/`);
    await page.waitForLoadState("networkidle");
  ...
  test("all buttons on login page should have accessible names", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/login`);
    await page.waitForLoadState("networkidle");
...
test.describe("Accessibility Tests - Keyboard Navigation", () => {
  test("should be able to navigate home page with keyboard", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/`);
    await page.waitForLoadState("networkidle");
  ...
  test("should be able to submit login form with Enter key", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/login`);
    await page.waitForLoadState("networkidle");
...
test.describe("Accessibility Tests - Heading Structure", () => {
  ...
  test("pricing page should have proper heading hierarchy", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/pricing`);
    await page.waitForLoadState("networkidle");
  ...
  test("headings should not skip levels", async ({ page }) => {
    ...
    for (const heading of headings) {
      if (heading.level > previousLevel + 1 && previousLevel !== 0) {
        ...
        console.log(
          `Warning: Heading level skipped from h${previousLevel} to h${heading.level}`,
        );
      }
...
test.describe("Accessibility Tests - Color Contrast", () => {
  test("text on home page should have sufficient contrast", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/`);
    await page.waitForLoadState("networkidle");
...
test.describe("Accessibility Tests - Focus States", () => {
  test("focusable elements should have visible focus indicator", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/login`);
    await page.waitForLoadState("networkidle");
...
test.describe("Accessibility Tests - Skip Links", () => {
  test("page should have skip link for keyboard users", async ({ page }) => {
    ...
    const focusedHref = await focusedElement
      .getAttribute("href")
      .catch(() => "");
    ...
    if (!isSkipLink) {
      console.log(
        "No skip link detected - consider adding one for accessibility",
      );
    }
    ...
    for (const button of buttons) {
      ...
      if (box) {
        ...
        if (!isLargeEnough) {
          const text = await button.textContent();
          smallTargets.push(
            `${text?.substring(0, 20)} (${box.width}x${box.height})`,
          );
        }
      }
    }
