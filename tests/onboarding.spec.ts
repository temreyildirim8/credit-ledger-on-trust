...
/**
 * E2E Tests for Ledgerly Onboarding Flow
...
test.describe("Onboarding Flow", () => {
  ...
  test.describe("Currency Selection Step", () => {
    ...
    test("should display currency options", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: /currency|select.*currency/i }),
      ).toBeVisible();
  ...
  test.describe("Language Selection Step", () => {
    ...
    test("should display language options", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: /language|select.*language/i }),
      ).toBeVisible();
  ...
  test.describe("Category Selection Step", () => {
    ...
    test("should display business category options", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: /category|business|type/i }),
      ).toBeVisible();

      // Check for category icons and names
      await expect(
        page.getByText(/bakkal|kirana|warung|spaza|kiosk/i),
      ).toBeVisible();
    ...
    test("should select category on click", async ({ page }) => {
      ...
      const retailButton = page
        .locator("button")
        .filter({ hasText: /retail/i })
        .first();
  ...
  test.describe("Navigation Flow", () => {
    test("should enable next button only when selection is made", async ({
      page,
    }) => {
      await page.goto(`${BASE_URL}/onboarding`);
      // Skip if redirected to login
