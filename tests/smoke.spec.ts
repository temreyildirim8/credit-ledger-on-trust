...
/**
 * Whole App Smoke Tests for Global Ledger
 *
...
test.describe('Smoke Tests - Protected Routes Require Authentication', () => {
  for (const route of PROTECTED_ROUTES) {
    test(`protected route ${route} should redirect to login when not authenticated`, async ({ page, context }) => {
...
test.describe('Smoke Tests - i18n Locales Work', () => {
  for (const locale of SUPPORTED_LOCALES) {
    test(`locale ${locale} should return 200 on home page`, async ({ page }) => {
      const response = await page.goto(`http://localhost:3000/${locale}/`);

      expect(response!.status()).toBe(200);
    });

    test(`locale ${locale} should display translated content`, async ({ page }) => {
...
test.describe('Smoke Tests - All Modals Open and Close', () => {
  ...
  test('add customer modal should open from customers page', async ({ page }) => {
    await page.goto(`${BASE_URL}/customers`);
    await page.waitForLoadState('networkidle');

    // Look for "Add Customer" or similar button
    const addButton = page.getByRole('button', { name: /add.*customer|new.*customer|create/i }).first();
  ...
  test('add transaction modal should open from transactions page', async ({ page }) => {
    await page.goto(`${BASE_URL}/transactions`);
    await page.waitForLoadState('networkidle');

    // Look for "Add Transaction" or similar button
    const addButton = page.getByRole('button', { name: /add.*transaction|new.*transaction|add.*debt|record.*payment/i }).first();
    const hasAddButton = await addButton.isVisible().catch(() => false);
...
test.describe('Smoke Tests - Navigation Works', () => {
  ...
  test('sidebar navigation should work on desktop', async ({ page }) => {
    ...
    if (hasSidebar) {
      ...
      const customersLink = sidebar.getByRole('link', { name: /customer/i }).first();
...
test.describe('Smoke Tests - Empty States Display', () => {
  ...
  test('customers page should show content or empty state', async ({ page }) => {
    await page.goto(`${BASE_URL}/customers`);
    await page.waitForLoadState('networkidle');
  ...
  test('transactions page should show content or empty state', async ({ page }) => {
    await page.goto(`${BASE_URL}/transactions`);
    await page.waitForLoadState('networkidle');
...
test.describe('Smoke Tests - Loading States', () => {
  test('pages should show loading state or content within reasonable time', async ({ page }) => {
...
test.describe('Smoke Tests - Accessibility Basics', () => {
  ...
  test('buttons should have accessible names', async ({ page }) => {
    ...
    for (const button of buttons) {
      const name = await button.getAttribute('aria-label').catch(() => null);
      const text = await button.textContent().catch(() => '');
      const hasAccessibleName = name !== null || text!.trim().length > 0;
